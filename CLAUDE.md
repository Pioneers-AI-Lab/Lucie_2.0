...
3. **Data Synchronization Workflow**:
   ...
   import { db } from '@/db';
   import { founders } from '@/db/schemas/founder';
   import { founderAirtableFieldIds } from '@/lib/airtable-field-ids-ref';
...
- **Completed**:
  ...
  - Three schemas defined: `founders.ts`, `startups.ts`, `session-events.ts`
  - Field ID mappings for all three tables (`lib/airtable-field-ids-ref.ts`)
  - Migration infrastructure (migrations auto-generated in `migrations/` directory)
- **Pending**:
  - Data migration scripts (Airtable → Turso bulk import)
...
- `.env` file contains all credentials (not in .gitignore - be careful with git history)
- No `.env.example` file - document all required variables in CLAUDE.md
...
- All secrets in `.env` file (no `.env.example` - document variables here instead)
