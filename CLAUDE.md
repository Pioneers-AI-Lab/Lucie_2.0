...
- It's marked as "LEGACY" and is **not actively used** by the agent (not included in agent's tool configuration - see `lucie-agents.ts`)
- The agent only uses the three specialized Turso tools listed above
- It provides global search across all Turso tables for cross-table queries if needed in the future
...
**Airtable → Turso Database Architecture**:

The project has completed migration from direct Airtable querying to a Turso database with structured schemas:

1. **Airtable (Source of Truth for Data Sync)**:
   - **Pioneers Cohort Data** (`SU_2025_BASE_ID`/`SU_2025_TABLE_ID`):
     - Contains information about the Pioneers accelerator program
     - Includes pioneer profiles, sessions, events, startups
     - Synced to Turso via `pnpm db:sync` command (incremental updates)
     - **Not queried directly by the agent** - only used for periodic data syncs
...
2. **Turso Database (Primary Data Source - Production Ready)**:
...
- **Import Conventions**:
  - Always use `.js` extensions in imports (e.g., `from './file.js'`) even though source files are `.ts`
  - This is required for ESM compatibility
  - Example: `import { db } from '../db/index.js';` (not `index.ts`)
...
- **Before making changes to tools or schemas**: Always check TODO.md for known issues that may affect your work
- **When adding new fields to schemas**: Update both the Drizzle schema AND the field ID mappings in `lib/airtable-field-ids-ref.ts`
...
- **Important**:
  ...
  - Configure table IDs in `.env` (see `docs/database-sync-guide.md` for detailed instructions)
  - **Reference**: See `docs/database-sync-guide.md` for complete sync workflow, troubleshooting, and automation options
...
- Fast, reliable query performance (typically <100ms)
- No rate limits (database connection, not external API)
- Specialized search functions for founders, sessions, and startups
- All agent queries exclusively use Turso database via three specialized tools
- Can handle concurrent queries without throttling
...
// Debug tool execution in agent
// Check terminal output when using pnpm dev:cli
// Look for tool call indicators: 🔧 Tool calls

// Debug Slack issues
// Check src/mastra/slack/verify.ts for signature verification
// Logs appear in terminal when running pnpm dev
```

### Architecture Decision Records

**Why Turso instead of direct Airtable queries?**
- Airtable has rate limits (5 requests/second)
- Turso queries are <100ms vs 500-2000ms for Airtable
- No API rate limits on Turso
- Type-safe queries with Drizzle ORM
- Airtable remains source of truth for data entry

**Why three separate query tools instead of one?**
- Each tool optimized for its domain (founders, sessions, startups)
- Specialized search types per data type
- Clearer tool selection for LLM
- Better error isolation and debugging
- Simpler to maintain and test

**Why beta version of Mastra?**
- GA version not released yet
- Beta provides needed functionality
- Active development and support
- Migration path planned (see TODO.md #20)
