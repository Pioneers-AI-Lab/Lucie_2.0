...
- **Known inconsistency**: Agent instructions (`lucie-instructions.ts`) mention `getCohortDataTool` which is no longer used by the agent - the agent only uses the three specialized Turso tools. Instructions also reference an AI Lab tool that doesn't exist yet.
